...
# TODO: better error handling when parsing secret policies. If we fail on one, indicate that we failed and go to the next.
# TODO: the message indicating that we might be vulnerable to automatic device approval may be misleading in case we are using an existing device

# TODO: formatting output
# TODO: put CLIENT in a separate collection with a specific task sequence with credentials, to illustrate the scenario in which we export the certificate from a compromised machine and use it to run SCCMSecrets.py
...
Les cas de figure :
 - Unauthenticated
    - Exploiting anonymous distribution point connection -> dump package scripts (but not secret policies).
    - Exploiting automatic device approval -> dump secret policies (but not package scripts when NAA account not retrieved in policy - enhanced HTTP auth not yet implemented).
 - Domain credentials but no machine account credentials
    - Dump package scripts (but no secret policies)
    - Exploiting automatic device approval -> dump secret policies
 - Machine account credentials
    - Secret policies + package scripts retrieval.
'''
...
1. The user did not provide any kind of credentials, and anonymous distribution point access is not enabled
No credentials were provided, and target distribution point does not accept anonymous access. In these conditions, we may:
 a. Try to register an SCCM client in order to exploit automatic device approval if it is configured on the SCCM site (this misconfiguration is not present by default). If successful, secret policies will be retrieved. If said policies contain NAA credentials, these will be used to download package files with extensions #FIXME.

Do you want to attempt registering a client (OPSec consideration: we will not be able to remove the client afterwards) ? [Y/n]


2. The user did not provide any kind of credentials, and anonymous distribution point access is enabled
No credentials were provided, but target distribution point does accept anonymous access. In these conditions, we may:
 a. Try to register an SCCM client in order to exploit automatic device approval if it is configured on the SCCM site (this misconfiguration is not present by default). If successful, secret policies will be retrieved.
 b. Download package files with extensions #FIXME.

Do you want to attempt registering a client (OPSec consideration: we will not be able to remove the client afterwards) ? If no, package file download will still be performed. [Y/n]


3. The user provided domain credentials, but not the ones of a machine account
Domain user account credentials were provided, but no machine account credentials. In these conditions, we may:
 a. Try to register an SCCM client in order to exploit automatic device approval if it is configured on the SCCM site (this misconfiguration is not present by default). If successful, secret policies will be retrieved.
 b. Download package files with extensions #FIXME.
...
4. The user provided machine account credentials
Machine account credentials provided. In these conditions, we may:
 a. Register an SCCM client to retrieve secret policies.
 b. Download package files with extensions #FIXME

Do you want to register a client (OPSec consideration: we will not be able to remove the client afterwards) ? If no, package file download will still be performed. [Y/n]
'''
